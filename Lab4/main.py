...
class Graph:
    ...
    def floyd_warshall(self):
        """
        Implementation of Floyd-Warshall algorithm to find the shortest paths
...
def visualize_graph(graph, algorithm="Both", sparse_or_dense=""):
    ...
    edge_labels = {(i, j): graph.graph[i][j] for i in range(graph.V) for j in range(graph.V) 
                  if i != j and graph.graph[i][j] != float('inf')}
...
def compare_algorithms(vertex_range=range(10, 201, 20)):
    """
    Compare Dijkstra and Floyd-Warshall algorithms on sparse and dense graphs
    with varying number of vertices
    """
...
def analyze_complexity_vs_theory():
    """
    Analyze the empirical complexity against theoretical complexity
    """
...
def test_algorithm_correctness():
    """
    Test the correctness of the Dijkstra and Floyd-Warshall implementations
    """
...
def optimize_sparse_graph_parameters():
    """
    Analyze how different edge probabilities affect algorithm performance on sparse graphs
    """
    vertices = 100
